...
unsigned int Speed = 0, Speedchange = 0;             // For *
unsigned int Voltage = 0, Voltagechange = 0;             // For *
...
//    while (OSCCONbits.LOCK != 1);
    ...
    PR1 = 14000;        // Timer1 period register 500Hz
    TMR1=0;             //Timer1 reset
    // </editor-fold>
    ...
    IPC7bits.U2RXIP = 2;        // Maximum priority
...
//    RPOR3bits.RP41R = 0b000011;             // Connect RP41 to U2TX
    ...
    IPC8bits.SPI2IP = 6; // INT Priority
...
int main( void )
{
    ...
    while(1)
    {
        if (flag_main)
        {
            ...
            SPITXRX(333);
            varCPU[1] = (char)(DATASPI[0] & 0x00FF);
            varCPU[2] = (char)(DATASPI[0] >>8);
            varCPU[3] = (char)(DATASPI[1] & 0x00FF);
            varCPU[4] = (char)(DATASPI[1] >>8);
            SendDataCPU();
            if (goflag != gotemp || Speedchange || Voltagechange)
            {
                Speedchange = 0;
                Voltagechange = 0;
                SPITXRX(goflag);
