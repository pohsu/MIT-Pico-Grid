...
unsigned char dataCPU[4];       // Variables from User Interface (Computer)
...
void SendDataMG(char address, char instruction)
{
    D_EN = 1;
...
void Configuration(void)
{
    ...
    IPC0bits.T1IP = 7;  // Timer1 Interrupt priority level=4
    IEC0bits.T1IE = 1;  // Enable Timer1 interrupt
    PR1 = 16000;       // Timer1 period register
    // </editor-fold>
    ...
    IPC7bits.U2RXIP = 5;        // Maximum priority
...
void Initialization(void)
{
    ...
    seq_device[0] = 1; //grid PMU
    seq_device[1] = 3; //Diesel PMU
    seq_device[2] = 4; //Inverter PMU
    seq_device[3] = 10; //Diesel Controller
    seq_device[4] = 21; //Inverter
    
    seq_inst[0] = 2; // 2 - send data that you have
    seq_inst[1] = 2;
    seq_inst[2] = 2;
    ...
    // <editor-fold defaultstate="collapsed" desc="Initialize Global Interruptions">
    INTCON2bits.GIE = 1; 
    // </editor-fold>
    ...
    while(1)
    {
        ...
        if (flag_main)
        {
            ...
            if (device > 4)
            {
                ...
                SendDataCPU();
//              if (InstBuffer[0] != 0)
//                  SendDataMG(InstBuffer[0],InstBuffer[1]);
//              for (kk = 0; kk < 50; kk++)
//                  Nop();
//              if (InstBuffer[2] != 0)
//                  SendDataMG(InstBuffer[2],InstBuffer[3]);
//              for (kk = 0; kk < 50; kk++)
//                   Nop();
//               else
//                {
...
//                 }
            ...
            SendDataMG(seq_device[device], seq_inst[device]);       // Call new device
        ...
        if (flag_MG)
        {
            LED5 = ~LED5;                      // Toggle LED 5
            
            if ((dataMG[0] == 111)&&(dataMG[11] == 222))    // If the frame is correct
            {
                response = 1;                               // Indicate that package was received correctly
        ...
        if (flag_CPU)                       // Reception from CPU
        {
            ...
            varSta[0] = dataCPU[0];         // Write received data into memory
            varSta[1] = dataCPU[1];
            varSta[2] = dataCPU[2];
            varSta[3] = dataCPU[3];

            if (varSta[0] != 0)             // If there is an instruction
            {
                InstBuffer[0] = varSta[0];      // Read the instruction
                InstBuffer[1] = varSta[1];
            }
            if (varSta[2] != 0)
            {
                InstBuffer[2] = varSta[2];
                InstBuffer[3] = varSta[3];
            }
        }
        // </editor-fold>
