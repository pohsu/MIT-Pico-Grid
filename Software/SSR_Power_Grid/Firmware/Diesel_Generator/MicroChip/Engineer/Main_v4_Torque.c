...
//    V[0] = ADC1BUF5;
//    V[1] = ADC1BUF6;
//    V[2] = ADC1BUF7; //not required for single phase
    IFS0bits.AD1IF = 0;
}
...
//    while (OSCCONbits.LOCK != 1);
    ...
    AD1CON1bits.SIMSAM = 0;         // Enable simultaneous sampling
...
int main( void )
{
    ...
    _Q16 Iref2 = _Q16ftoi(0);  //Iref

    // Current Loop PI controller 1 Y(k) = u0*U(k)+u1*U(k-1)+y1*Y(k-1)
    _Q16 Io1, error1, output1, duty1Q16 = _Q16ftoi(0);
    _Q16 Kp01 = _Q16ftoi(0.01);
    _Q16 Ki01_default = _Q16ftoi(20*0.0005); //gain*T
    _Q16 Ki01 = Ki01_default; //gain*T
    _Q16 integ01, integ01_pre = 0;

    // Current Loop PI controller 2 Y(k) = u0*U(k)+u1*U(k-1)+y1*Y(k-1)
    _Q16 Io2, error2, output2, duty2Q16 = _Q16ftoi(0);
    _Q16 Kp02 = _Q16ftoi(0.01);
    _Q16 Ki02_default = _Q16ftoi(20*0.0005); //gain*T
    ...
    _Q16 Q1_169 = _Q16ftoi(0.005917159);
    // </editor-fold>
    ...
    while(1)
    {
        if (flag_main)
        {
            ...
            // <editor-fold defaultstate="collapsed" desc="Current controller 1">
            Iref1 = _Q16ftoi(voltageref*0.1);
            Io1 = _Q16ftoi(I[0]); //Remove offset
            Io1 = _Q16mac(Io1, Q1_169, 0);
            error1 = Iref1 - Io1;
            ...
            // <editor-fold defaultstate="collapsed" desc="Current controller 2">
            Iref2 = _Q16ftoi(speedref*0.1);
            Io2 = _Q16ftoi(I[1]); //Remove offset
            Io2 = _Q16mac(Io2, Q1_169, 0);
            error2 = Iref2 - Io2;
            //Y(k) = u0*U(k)+u1*U(k-1)+y1*Y(k-1)
            ...
            if (output2 > _Q16ftoi(0.95))
            {
                output2 = _Q16ftoi(0.95);
                Ki02 = _Q16ftoi(0);
            }
            else
            {
            ...
            duty2Q16 = _Q16mac(output2, _Q16ftoi(7000), 0);
            Duty2 = duty2Q16 >> 16;
            // </editor-fold>
...
//                integ01_pre = -1*integ01_pre;
            ...
            VIW[0] = (Io1*10)>>16; //Voltage Magnitude
            VIW[1] = (Io2*10)>>16; //Voltage Magnitude
            VIW[2] = speed_Hz>>16;
            kk++;
            ctr = ctr+1;
