...
//! \e usCPU2IpcInterrupt and \e usCPU1IpcInterrupt parameters can be one of the
//! following values:
...
//*****************************************************************************
void
IPCInitialize (volatile tIpcController *psController, uint16_t usCPU2IpcInterrupt,
...
//! read command to the remote CPU system and set a ResponseFlag to track the
//! status of the read.
//! The remote CPU system will respond with a DataWrite command which will place
//! the data in the local CPU address pointed to by \e pvData. When the local CPU
//! receives the DataWrite command and writes the read data into \e *pvData location, it
...
//*****************************************************************************
//! Sets the designated bits in a 16-bit write-protected data word at the remote
//! CPU system address
//!
//! \param psController specifies the address of a \e tIpcController instance
//! used to store information about the "Put" and "Get" circular buffers and
...
//! used to store information about the "Put" and "Get" circular buffers and
//! their respective indexes.
//! \param ulAddress specifies the seconadry CPU address to write to
//! \param ulMask specifies the 16/32-bit mask for bits which should be cleared
//! at \e ulAddress. 16-bit masks should fill the lower 16-bits (upper 16-bits
//! will be all 0x0000).
//! \param usLength specifies the length of the bit mask (1=16-bits, 2=32-bits)
...
//! remote cpu only when this function is called in response to \e IPCMtoCDataRead().
//! Otherwise, set to 0.
//!
...
//! used to store information about the "Put" and "Get" circular buffers and
//! their respective indexes.
//! \param ulAddress specifies the write-protected remote CPU address to write to
...
//! to read from.
//! \param ulShareAddress specifies the local CPU shared memory address the read
//! block will read to.
...
//!  /e ulShareAddress. (\e ulResponseFlag MUST use IPC flags 17-32, and not 1-16)
//!
                 ...
                 uint32_t ulResponseFlag)
{
    ...
    // where block data should be read to (corresponding to local CPU ulShareAddress).
...
//!
//! This function will allow the local CPU system to write a block of data to
//! a write-protected region on the remote CPU system starting from the location
//! specified by the \e ulAdress parameter. Prior to calling this function,
//! the local CPU system code should place the data to write in shared memory
...
//! to pass to the function. The \e bBlock parameter can be one of the following
//! values: \b ENABLE_BLOCKING or \b DISABLE_BLOCKING.
//!
//! \return status of command (\b STATUS_PASS =success, \b STATUS_FAIL = error
//! because PutBuffer was full, command was not sent)
...
//! used to store information about the "Put" and "Get" circular buffers and
//! their respective indexes.
//! \param ulMask specifies the 32-bit mask for the GSxMSEL RAM control register
//! to indicate which GSx SARAM blocks the slave CPU is requesting master access to.
...
//! IPCClearBits_Protected() functions, and therefore in the master CPU application
//! code, the corresponding functions should be called.
//! The \e bBlock parameter can be one of the following values: \b
//! ENABLE_BLOCKING or \b DISABLE_BLOCKING. The \e usMaster parameter can be
//! either: \b IPC_GSX_CPU2_MASTER or \b IPC_GSX_CPU1_MASTER. The \e ulMask parameter
                     ...
                     uint16_t usMaster, uint16_t bBlock)
{
    ...
    if (usMaster == IPC_GSX_CPU2_MASTER)
    {
        ...
        {
            status =
                IPCLtoRSetBits_Protected (psController, GSxMSEL_REGaddress, ulMask,
                                          IPC_LENGTH_32_BITS,
                                          bBlock);
        }
    }
    else if (usMaster == IPC_GSX_CPU1_MASTER)
    {
        ...
        {
            status =
                IPCLtoRClearBits_Protected (psController, GSxMSEL_REGaddress, ulMask,
                                            IPC_LENGTH_32_BITS,
                                            bBlock);
        }
    }
...
//! response flag tracking the read so other threads in the local CPU system will 
//! be aware that the data is ready.
//!
//!
//! \return None.
//*****************************************************************************
...
//!
//! This function will allow the local CPU system to write a 16/32-bit word
//! received from the secondary CPU system to the write-protected address indicated
//! in \e *psMessage.
...
//! word at the local CPU address specified in /e psMessage, and send a Write command
//! with the read data back to the local CPU system. It will also send the
//! Response Flag used to track the read back to the remote CPU.
//! The \e bBlock parameter can be one of the following values: \b
//! ENABLE_BLOCKING or \b DISABLE_BLOCKING.
//!
//! \return None.
                ...
                uint16_t bBlock)
{
    ...
    // and cast as 32-bit word to send back to remote CPU.
    ...
    // Send a Write command to write the requested data to the remote CPU read into
    // address.
    // psMessage->uldataw2 contains remote CPU address where readdata will be written.
...
//! Protected command with the read data back to the remote CPU system at a write
//! protected address. It will also send the Response Flag used to track the
//! read back to the remote CPU. The \e bBlock parameter can be one of the following
//! values: \b ENABLE_BLOCKING or \b DISABLE_BLOCKING.
//!
//! \return None.
//*****************************************************************************
                          ...
                          uint16_t bBlock)
{
    ...
    // If data word length = 16-bits, read the 16-bit value at the given address
    // and cast as
    // 32-bit word to send back to remote CPU.
    // If data word length = 32-bits, read the 32-bit value at the given
    // address.
    //
    usLength = (uint16_t)psMessage->uldataw1;
    ...
    IPCLtoRDataWrite_Protected(psController, psMessage->uldataw2, ulReaddata,
                               usLength, bBlock,(psMessage->uldataw1 & 0xFFFF0000));
}
...
//! remote CPU system.
//!
//! This function will allow the remote CPU system to set the bits in a 16/32-bit
//! word on the local CPU system via a local CPU address and mask passed in via
//! the \e psMessage.
//!
//! \return None.
//*****************************************************************************
...
IPCRtoLSetBits(tIpcMessage *psMessage)
{
    ...
    if (usLength == IPC_LENGTH_16_BITS)
    {
        *(volatile uint16_t*)psMessage->uladdress |= (uint16_t) psMessage->uldataw2;
    }
    else if (usLength == IPC_LENGTH_32_BITS)
    {
...
//! protected 16/32-bit word on the local CPU system via a local CPU address and
...
//! system.
//! The data is first written by the remote CPU to shared RAM. This function reads the
//! shared RAM location, word size (16- or 32-bit), and block size from \e
//! psMessage and writes the block to the local address specified in \e psMessage.
//!
//! \return None.
//*****************************************************************************
void
...
//! remote CPU to shared RAM. This function reads the shared RAM location, word size
//! (16- or 32-bit), and block size from \e psMessage  and writes the block to
//! the local address specified in \e psMessage.
//!
//! \return None.
//*****************************************************************************
void
...
//! with a single optional parameter. There is no return value from the executed
//! function.
//!
//! \return None.
//*****************************************************************************
